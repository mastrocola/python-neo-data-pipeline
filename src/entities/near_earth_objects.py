...
from pyspark.sql.types import ArrayType, BooleanType, DateType, DecimalType, DoubleType, FloatType, StringType, StructField, StructType
from pyspark.sql.utils import AnalysisException
from src.settings import NASA_API_KEY, NASA_NEO_API_URL, PROCESSED_FOLDER, RAW_FOLDER
from src.utils.folder import verify_folder_and_create_if_not_exists
...
class NeoWsDataPipeline:
    ...
    def acquire_feed_data(self):
        ...
        for key, value in api_data['near_earth_objects'].items():
            with open(f'{RAW_FOLDER}/{key}.json', 'w', encoding='utf-8') as json_file:
                json.dump(value, json_file)

    def process_data(self):
        try:
            with open(f'{RAW_FOLDER}/{self.__search_at}.json', 'r', encoding='utf-8') as raw_file:
